...
app.get('/api/tenants', cors(), authenticateMultiTenantAdmin, async (req, res) => {
  try {
    const tenants = await Tenant.find({ deleted_at: null })
      .select('name slug contact_email suspended createdAt')
      .sort({ createdAt: -1 });
...
app.patch('/api/tenants/:slug/toggle-status', cors(), authenticateMultiTenantAdmin, async (req, res) => {
  try {
    ...
    res.json({ 
      message: `Institution ${tenant.suspended ? 'suspended' : 'activated'} successfully`,
      suspended: tenant.suspended
    });
...
app.get('/api/tenant/:slug/profile', cors(), authenticateMultiTenantAdmin, async (req, res) => {
  try {
    const { slug } = req.params;
    const tenant = await Tenant.findOne({ slug, deleted_at: null });
...
app.put('/api/tenant/:slug/profile', cors(), authenticateMultiTenantAdmin, async (req, res) => {
  try {
    const { slug } = req.params;
    ...
    const allowedFields = ['name', 'address', 'contact_email', 'contact_phone', 'timezone', 'logo_url'];
